...
void ReliableMailChunkerTest(int farAddr) {
    PacketHeader outPktHdr, inPktHdr;
    MailHeader outMailHdr, inMailHdr;
    const char *data = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam fringilla ullamcorper enim in dapibus. Nam mattis, mi ut faucibus auctor, nunc libero rhoncus enim, sed laoreet est metus ut ipsum. Aliquam a metus vitae nunc faucibus commodo vel eu nisl. Quisque in luctus risus. Maecenas sed enim efficitur, malesuada nibh tempor, molestie dui. Cras eros ligula, bibendum eget eros eget, pharetra pharetra massa. Morbi risus lectus, imperdiet congue elit ac, molestie convallis metus. Duis accumsan porta dui. Vestibulum efficitur erat et sapien blandit sodales. Suspendisse tempor pharetra eleifend. Mauris pretium finibus enim, sed placerat dui vehicula ut. Integer elementum sapien in lacus vestibulum, vel ultrices ex imperdiet. Vivamus id luctus neque. Suspendisse aliquam augue massa, nec aliquet urna imperdiet id. Donec quis sem lorem. Maecenas porta leo eu molestie facilisis. Duis vel odio nec tortor aliquam mollis eget quis magna. Vestibulum ante ipsum primis in faucibus orci luctus orci.";
    ...
    printf("ReliableMailChunkerTest: Starting TCP-like test with machine %d\n", farAddr);
    fflush(stdout);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        // Send message (NON-BLOCKING - returns immediately with seqNum)
        unsigned int seqNum = reliablePost->SendReliableAnySize(outPktHdr, outMailHdr, data);
